...
/**
 * 实体[${de.logicName}]数据分页[${item.name}]部件模型
 */
...
<#if (dritem.getDataTreeId()??)&&(dritem.getDataTreeId()?length gt 0)>
        drCtrlItem${dritem_index?c}.setDataTreeId("${dritem.getDataTreeId()}");
</#if>
